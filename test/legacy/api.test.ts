...
describe("API", () => {
  describe("Admin API", () => {
    const requester = chai.request("http://localhost:8378/admin");
    let apiKey: string;
          ...
          it("It should fail to verify the API key", (done) => {
            requester
              ...
              .end((err, res) => {
                ...
                res.body.should.be.eql({ msg: "jwt malformed" });
                done();
              });
          });
          ...
          it("It should succesfully change the domainprefix", (done) => {
            requester
              ...
              .end((err, res) => {
                ...
                res.body.should.not.have.property("msg");
                res.body.should.have.property("domainPrefix");
                res.body.domainPrefix.should.equal("holmweb");
        ...
        describe("GET /service/categories/:apiKey", () => {
          it("It should succesfully get all categories previously registered, including updated information", (done) => {
            requester.get(`/service/categories/${apiKey}`).end((err, res) => {
              ...
              const filtered = res.body.map((category) => {
          ...
          it("It should succesfully fetch the calendar since it was just created and should verify all the updates", (done) => {
            requester
              ...
              .end((err, res) => {
                ...
                res.body.schedule.specialWeek[0].should.have.property(
                  "schedule"
                );
    ...
    const priceID = "price_1IPPu7JiYaX7uDQze19kV3LQ";
    let customerID;
    describe("Test upgrading and premium features", () => {
      ...
      describe("Authorization API", () => {
        it("It should verify that a stripeCustomerID exists", (done) => {
          requester.get(`/auth/verify-key/${apiKey}`).end((err, res) => {
            res.should.have.status(200);
  ...
  describe("Client API", () => {
    const requester = chai.request("http://localhost:8378/client");
    ...
    describe("GET /theme/:domainPrefix", () => {
      it("It should get the correct theme for the client", (done) => {
        requester
          .get(`/theme/${bookingSettings.domainPrefix}`)
          .end((err, res) => {
            res.should.have.status(200);
            ...
            res.body.theme.bookingSettings.domainPrefix.should.be.eql(
              bookingSettings.domainPrefix
            );
            done();
          });
      });
    ...
    describe("GET /available-times/:domainPrefix/:serviceID/:date", () => {
      it("It should fail to get the available times today - nonexistent serviceID", (done) => {
        requester
          .get(
            `/available-times/${
              bookingSettings.domainPrefix
            }/notaserviceID/${today.format("DD-MM-YYYY")}`
          )
          .end((err, res) => {
            assertUserError(res);
            done();
      ...
      it("It should get the available times today", (done) => {
        requester
          .get(
            `/available-times/${bookingSettings.domainPrefix}/${
              serviceIDs[0]
          ...
          .end((err, res) => {
            ...
            const todaysSchedule = specialSchedule.schedule.filter(
              (day) => day.day === today.day()
            )[0].schedule;
            let { startOfWork, endOfWork } = todaysSchedule;

            startOfWork = today
              .set("hour", startOfWork.hour)
              .set("minute", startOfWork.minute)
              .set("seconds", 0)
              .set("ms", 0);
            endOfWork = today
              .set("hour", endOfWork.hour)
              .set("minute", endOfWork.minute)
              .set("seconds", 0)
              .set("ms", 0);

            const numberOfReceivedTimes = res.body[0].availableTimes.length;

            const actualValidTimes = res.body[0].availableTimes.filter(
              (time) => {
                const { startTime, endTime } = time;

                if (
                  dayjs.utc(startTime).isSameOrAfter(startOfWork) &&
                  dayjs.utc(endTime).isSameOrBefore(endOfWork)
                ) {
                  if (
                    dayjs
                      .utc()
      ...
      it("It should get the available times a day in the future", (done) => {
        requester
          .get(
            `/available-times/${bookingSettings.domainPrefix}/${
              serviceIDs[0]
            }/${today.add(14, "days").day(3).format("MM-DD-YYYY")}`
          )
          .end((err, res) => {
            res.should.have.status(200);
        ...
        requester
          .get(
            `/available-times/${bookingSettings.domainPrefix}/${
              ...
              .add(bookingSettings.maxDaysBookAhead + 1, "days")
              .format("MM-DD-YYYY")}`
          )
          .end((err, res) => {
            res.should.have.status(200);
            res.body.forEach((calendar) =>
              calendar.availableTimes.length.should.be.eql(0)
            );
            done();
          });
      });
      ...
      it("It should succesfully book an appointment", (done) => {
        requester
          ...
          .end((err, res) => {
            res.should.have.status(200);
            res.body.should.not.have.property("msg");
            res.body.startTime.should.be.eql(
              today.add(14, "days").day(3).hour(10).minute(30).toJSON()
            );
            res.body.date.should.be.eql(
              today
                .add(14, "days")
                .day(3)
                .hour(10)
      ...
      it("It should succesfully book another appointment 2 hours later", (done) => {
        requester
          ...
          .end((err, res) => {
            ...
            res.body.should.not.have.property("msg");
            res.body.startTime.should.be.eql(
              today.add(14, "days").day(3).hour(12).minute(30).toJSON()
            );
            res.body.date.should.be.eql(
              today
                .add(14, "days")
