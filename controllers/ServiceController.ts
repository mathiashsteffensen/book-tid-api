...
export default class ServiceController {
  ...
  static updateCategory: MyRequestHandler = async (req, res) => {
    ...
    const category = await ServiceCategory.findById(req.body.id)
      .exec()
      .catch((err) => {
        throw new ServerError(err);
      });
    ...
    await Promise.all(
      servicesToUpdate.map(async (serviceToUpdate) => {
        return await serviceToUpdate
          .updateOne({ categoryName: req.body.name })
          .exec();
      })
    );
  ...
  static deleteCategory: MyRequestHandler = async (req, res) => {
    ...
    // Deletes category
    const category = await ServiceCategory.findByIdAndDelete(req.body.id).catch(
      (err) => {
        throw new ServerError(err);
      }
    );
