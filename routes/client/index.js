...
const { obeysBookingRestrictions } = require("../../db/queries");
  ...
  async (req, res, next) => {
    if (req.client) {
      if (req.client.activatedApps.includes("clientUiBranding")) {
        ...
        res.json({ theme: req.client, branding: app });
      } else {
        res.json({ theme: req.client, branding: false });
      }
    } else {
      res.status(404);
...
clientRouter.get(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const user = await new UserReaderService({
        userId: req.client.id,
      }).withSubscription();

      const canBookThisMonth = await obeysBookingRestrictions(
        user,
        dayjs(date).add(12, "hours").toJSON().slice(0, 10)
      );
  ...
  async (req, res, next) => {
    ...
    if (!service) {
      ...
      if (!customer.name) {
        res.status(400);
        return next({
          msg: "Indtast venligst et navn",
        });
      }
      if (!customer.email)
        return next({
          msg: "Indtast venligst en email",
        });

      if (!customer.phoneNumber)
        return next({
          msg: "Indtast venligst et gyldigt telefonnummer",
        });
                  ...
                  async (err, appointment) => {
                    if (err) throw new Error("Der skete en fejl.");
                      ...
                      if (req.client.activatedApps.includes("textReminder")) {
                        ...
                        if (!textReminderApp || !textReminderApp.sendReminders)
                          return;

                        const { _id, stripeCustomerID } =
                          await AdminClient.findOne({
                        ...
                        const sendAt = dayjs
                          .utc(appointment.startTime)
                          .subtract(1, "day")
                          .set("hours", textReminderApp.remindAt.split(":")[0])
                          .set(
                            "minutes",
                            textReminderApp.remindAt.split(":")[1]
                          )
                          .subtract(1, "hour")
                          .unix();
                      ...
                      Appointment.create(
                        ...
                        async (err, appointment) => {
                          if (err) throw new Error("Der skete en fejl.");
                            ...
                            if (
                              req.client.activatedApps.includes("textReminder")
                            ) {
                              console.log("gonna try to schedule a text");
                              const textReminderApp =
                                await TextReminderApp.findOne({
                                  adminEmail: req.client.email,
                                  activated: true,
                                }).exec();
                              ...
                              const sendAt = dayjs
                                ...
                                .set(
                                  "hours",
                                  textReminderApp.remindAt.split(":")[0]
                                )
                                .set(
                                  "minutes",
                                  textReminderApp.remindAt.split(":")[1]
                              ...
                              await sendTextReminder({
                                businessName:
                                  req.client.businessInfo.name.replace(
                                    ".",
                                    " "
                                  ),
                                sendAs: textReminderApp.sendAs,
...
clientRouter.get(
  ...
  async (req, res, next) => {
    try {
      const personalDataPolicy = await AdminClient.findOne({
        email: req.client.email,
      })
