...
export const sendConfirmationEmail = async (customerEmail, templateData) => {
...
export const sendNewBookingEmail = async (adminEmail, templateData) => {
  if (process.env.NODE_ENV === "test" || process.env.NODE_ENV === "development") return
  const msg = {
    to: adminEmail,
    from: sendFrom,
  ...
  return sgMail.send(msg)
    .then(() => 
      console.log("New booking E-Mail sent"))
    .catch((err) => 
      console.log(err))
}

export const sendClientCancelEmail = async (adminEmail, templateData) => {
  if (process.env.NODE_ENV === "test" || process.env.NODE_ENV === "development") return
  const msg = {
    to: adminEmail,
    from: sendFrom,
  ...
  return sgMail.send(msg)
    .then(() => 
      console.log("Client cancel E-Mail sent"))
    .catch((err) => 
      console.log(err))
}

export const sendSignUpConfirmation = async (email, templateData) => {
  ...
  return sgMail.send(msg)
    .then(() => 
      console.log("Sign Up Confirmation E-Mail sent"))
    .catch((err) => 
      console.log(err))
}

export const sendNewEmailConfirmation = async (email, templateData) => {
  if (process.env.NODE_ENV === "test") return
  const msg = {
    to: email,
    from: sendFrom,
  ...
  return sgMail.send(msg)
    .then(() => 
      console.log("New Email Confirmation sent"))
    .catch((err) => 
      console.log(err))
}
